...
if($tipo==1){
	$result = $mysql->query("SELECT * from reparaciones WHERE id_usr = $id AND estado LIKE 'completado' limit $inicio,$cuantos");
...
<th scope="row"><?php echo $row['id'] ?></th>
    ...
    <td><a href="./factura.php?viewid=<?php echo $row['id'] ?>" style="color:black"type="button" class="btn btn-light">Ver Factura</button></td>
 </tr>
<?php
	}
}else{
  //si somos usuario 2 entonces tenemos que mostrar todos los registros
	$result = $mysql->query("SELECT * FROM reparaciones WHERE estado LIKE 'completado'  limit $inicio,$cuantos");
while($row = $result->fetch_assoc()) {
    ?>
<tr>
<th scope="row"><?php echo $row['id'] ?></th>
...
?>
  <ul class="pagination justify-content-center">
    ...
    <li class="page-item">
      <?php
     echo '<a class="page-link" href="./misfacturas.php?p='.$siguiente.'" style="color:black">Siguiente</a>';
      ?>
    </li>
  </ul>
</section>
